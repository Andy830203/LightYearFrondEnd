...
onMounted(() => {
    document.querySelectorAll('.enableDragable').forEach((tableBody) => {
        enableDragAndDrop(tableBody, 'li');
    });
})
</script>
...
<template>
    <div>
        <ul class="enableDragable">
            <li draggable="true" v-for="i in 10">{{ i }}</li>
        </ul>
    </div>
</template>
